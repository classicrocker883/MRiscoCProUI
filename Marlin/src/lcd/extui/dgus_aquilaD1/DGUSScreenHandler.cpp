...
void GetPreparePLAHoted(void);
void GetPreparePLABed(void);
void GetPrepareABSHoted(void);
void GetPrepareABSBed(void);
...
uint8_t load_flag=0,unload_flag=0;
...
void DGUSScreenHandler::DGUSLCD_SendMAX_ACC(DGUS_VP_Variable &var) {
  if (var.memadr) {
    switch (var.VP) {
        case VP_X_MAX_ACC:
                DEBUG_ECHOLNPGM(" VP_X_MAX_ACC ", *(int16_t *)var.memadr);
               dgusdisplay.WriteVariable(var.VP, *(int16_t*)var.memadr);

        break;
        case VP_Y_MAX_ACC:
                DEBUG_ECHOLNPGM(" VP_Y_MAX_ACC ", *(int16_t *)var.memadr);
                dgusdisplay.WriteVariable(var.VP, *(int16_t*)var.memadr);

        break;
        case VP_Z_MAX_ACC:
                DEBUG_ECHOLNPGM(" VP_Z_MAX_ACC ", *(int16_t *)var.memadr);
                dgusdisplay.WriteVariable(var.VP, *(int16_t*)var.memadr);

        break;
        case VP_E_MAX_ACC:
                DEBUG_ECHOLNPGM(" VP_E_MAX_ACC ", *(int16_t *)var.memadr);
                dgusdisplay.WriteVariable(var.VP, *(int16_t*)var.memadr);
...
void DGUSScreenHandler::DGUSLCD_SendPrintProgressToDisplay(DGUS_VP_Variable &var) {
  ...
  float tmp1= tmp / 100.0*232 ;

  LCD_SERIAL.write(0x5A);
  LCD_SERIAL.write(0xA5);
  LCD_SERIAL.write(0x11);
  LCD_SERIAL.write(0x82);

  LCD_SERIAL.write(0x5a);
  LCD_SERIAL.write(0x00);

  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0x04);

  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0x01);

  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0x14);
  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0xBB);

  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0x14+(uint16_t)(tmp1));

  LCD_SERIAL.write(0x00);
  LCD_SERIAL.write(0xCF);

  LCD_SERIAL.write(0xF8);
  LCD_SERIAL.write(0x00);
  FIL_RUNOUT_FLAG = READ(PA0);

  if(lcd_info.fil_runout_flag)
  {
...
// Send the current print progress to the display.
void DGUSScreenHandler::DGUSLCD_SendPrintProgressToDisplay_1(DGUS_VP_Variable &var) {
  uint16_t tmp = ExtUI::getProgress_percent();
  float tmp1= tmp / 100.0*232;
...
void DGUSScreenHandler::HandleFanChanged(DGUS_VP_Variable &var, void *val_ptr) {
  celsius_t newvalue = BE16_P(val_ptr);
  // celsius_t newvalue = swap16(*(uint16_t*)val_ptr);
  celsius_t acceptedvalue;
  // DEBUG_ECHOLNPGM("HandleFanChanged");
  // DEBUG_ECHOLNPGM("HandleFanChanged",acceptedvalue);
  // DEBUG_ECHOLNPGM("HandleFanChanged",newvalue);
  switch (var.VP) {
    default: return;
      case VP_Fan_SET:
        NOMORE(newvalue, FAN_MAX_PWM);
        thermalManager.set_fan_speed(0, newvalue);
        // acceptedvalue = newvalue;
        break;
        case VP_Tune_FAN_Set:
        NOMORE(newvalue, FAN_MAX_PWM);
        thermalManager.set_fan_speed(0, newvalue);
        // acceptedvalue = newvalue;
        break;
  ...
  {
    // DEBUG_ECHOLNPGM("send_to_display_handler");
  ...
  switch (var.VP) {
    default: return;
      ...
      case VP_E0_CONTROL:
        ...
        // snprintf_P(buf, 32, PSTR("M109 S%d"), newvalue);
        // queue.enqueue_one_now(buf);
        acceptedvalue = thermalManager.degTargetHotend(0);
        lcd_info.set_hoted_flag=1;
        lcd_info.reach_cnt = 0;
        break;
    #endif
    #if HAS_MULTI_HOTEND
      case VP_T_E1_Set:
...
void DGUSScreenHandler::HandleMotorLockUnlock(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  static bool look_flag=false;
  // const int16_t lock = swap16(*(uint16_t*)val_ptr);
  // strcpy_P(buf, lock ? PSTR("M18") : PSTR("M17"));
    if(look_flag==false)
    {
        dgusdisplay.WriteVariable(0x5200,(uint16_t)0);
        queue.enqueue_one_now(PSTR("M18"));
        look_flag =true;
    }
    else
    {
        dgusdisplay.WriteVariable(0x5200,(uint16_t)1);
        queue.enqueue_one_now(PSTR("M17"));
        look_flag =false;
    }
}

#define BEEP_FLAG_EEPROM_ADDRESS 0x10

void DGUSScreenHandler::HandleBeepLockUnlockSelect(void)
{
      BL24CXX::read(BEEP_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.beep_flag,sizeof(lcd_info.beep_flag));
    if(lcd_info.beep_flag==true)
    {
        dgusdisplay.WriteVariable(0x5210,(uint16_t)0);
        LCD_SERIAL.write(0x5A);
        LCD_SERIAL.write(0xA5);
        LCD_SERIAL.write(0x07);
        LCD_SERIAL.write(0x82);
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x80);
        LCD_SERIAL.write(0x5A);
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x3B);
    }
    else
    {
        dgusdisplay.WriteVariable(0x5210,(uint16_t)1);
        // queue.enqueue_one_now(PSTR("M17"));
        LCD_SERIAL.write(0x5A);
        LCD_SERIAL.write(0xA5);
        LCD_SERIAL.write(0x07);
        ...
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x33);
    }

}

void DGUSScreenHandler::HandleBeepLockUnlock(DGUS_VP_Variable &var, void *val_ptr) {

 DEBUG_ECHOLNPGM("HandleBeepLockUnlock");
  // static bool look_flag=false;
  // const int16_t lock = swap16(*(uint16_t*)val_ptr);
  // strcpy_P(buf, lock ? PSTR("M18") : PSTR("M17"));
    if(lcd_info.beep_flag==false)
    {
        dgusdisplay.WriteVariable(0x5210,(uint16_t)0);
        LCD_SERIAL.write(0x5A);
        LCD_SERIAL.write(0xA5);
        ...
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x3B);
        lcd_info.beep_flag =true;
        BL24CXX::write(BEEP_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.beep_flag,sizeof(lcd_info.beep_flag));
    }
    else
    {
        dgusdisplay.WriteVariable(0x5210,(uint16_t)1);
        ...
        LCD_SERIAL.write(0x00);
        LCD_SERIAL.write(0x33);
        lcd_info.beep_flag =false;
        BL24CXX::write(BEEP_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.beep_flag,sizeof(lcd_info.beep_flag));
    }
}

#define FILAMENT_FLAG_EEPROM_ADDRESS 0x63
void DGUSScreenHandler::filament_flag_select(void){
    BL24CXX::read(FILAMENT_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.fil_runout_flag,sizeof(lcd_info.fil_runout_flag));
    if( lcd_info.fil_runout_flag==false)
    {
         dgusdisplay.WriteVariable(0x5260,(uint16_t)1);
    }
    else
    {
         dgusdisplay.WriteVariable(0x5260,(uint16_t)0);
    }
}
void set_no_filament(void){
      lcd_info.fil_runout_flag=0;
      BL24CXX::write(FILAMENT_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.fil_runout_flag,sizeof(lcd_info.fil_runout_flag));
}
void set_filament(void){
      lcd_info.fil_runout_flag=1;
      BL24CXX::write(FILAMENT_FLAG_EEPROM_ADDRESS,(uint8_t*)&lcd_info.fil_runout_flag,sizeof(lcd_info.fil_runout_flag));
}

void DGUSScreenHandler::HandleCHECK_FIL(DGUS_VP_Variable &var, void *val_ptr) {
...
void DGUSScreenHandler::HandlePLAHEAT(DGUS_VP_Variable &var, void *val_ptr) {

    uint16_t temp_hoted,temp_bed;
    char cmd[32];
    GetPreparePLAHoted();
    GetPreparePLABed();

    if(lcd_info.preheat_pla_hoted==0||lcd_info.preheat_pla_hoted>300)
    {
        temp_hoted = 200;
    }
    else
    {
        temp_hoted = lcd_info.preheat_pla_hoted;
    }

    if(lcd_info.preheat_pla_bed==0||lcd_info.preheat_pla_bed>110)
    {
        temp_bed = 60;
    }
    else
    {
        temp_bed = lcd_info.preheat_pla_bed;
    }
    // sprintf_P(cmd, PSTR("M109S%i"), temp_hoted);
    // queue.enqueue_now_P(cmd);
    thermalManager.setTargetHotend(temp_hoted,0);
...
void DGUSScreenHandler::HandleABSHEAT(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    if(lcd_info.preheat_abs_hoted==0||lcd_info.preheat_abs_hoted>300)
    {
        temp_hoted = 230;
    }
    else
    {
        temp_hoted = lcd_info.preheat_abs_hoted;
    }
    if(lcd_info.preheat_abs_bed==0||lcd_info.preheat_abs_bed>110)
    {
        temp_bed = 80;
    }
    else
    {
        temp_bed = lcd_info.preheat_abs_bed;
    }
    thermalManager.setTargetHotend(temp_hoted,0);
    thermalManager.setTargetBed(temp_bed);
    thermalManager.set_fan_speed(0, 255);
...
void DGUSScreenHandler::DWIN_CompletedMaticHate(void)
{

        if(load_flag==1)
        {
            if(lcd_info.language == (language_t)1)
            GotoScreen((DGUSLCD_Screens)8);
            else if(lcd_info.language == (language_t)2)
            GotoScreen((DGUSLCD_Screens)48);
             else if(lcd_info.language == (language_t)3)
            GotoScreen((DGUSLCD_Screens)83);
             else if(lcd_info.language == (language_t)4)
            GotoScreen((DGUSLCD_Screens)123);
             else if(lcd_info.language == (language_t)5)
            GotoScreen((DGUSLCD_Screens)158);
             else if(lcd_info.language == (language_t)6)
            GotoScreen((DGUSLCD_Screens)198);
            ...
            gcode.process_subcommands_now(PSTR("G1 F100 E100"));
            gcode.process_subcommands_now(PSTR("G1 F100 E300"));
        ...
        {
           if(lcd_info.language == (language_t)1)
            GotoScreen((DGUSLCD_Screens)14);
            else if(lcd_info.language == (language_t)2)
            GotoScreen((DGUSLCD_Screens)55);
            else if(lcd_info.language == (language_t)3)
            GotoScreen((DGUSLCD_Screens)109);
            else if(lcd_info.language == (language_t)4)
            GotoScreen((DGUSLCD_Screens)148);
            else if(lcd_info.language == (language_t)5)
            GotoScreen((DGUSLCD_Screens)184);
            else if(lcd_info.language == (language_t)6)
            GotoScreen((DGUSLCD_Screens)221);
            gcode.process_subcommands_now(PSTR("G28XY"));
            gcode.process_subcommands_now(PSTR("G92 Z0 E0"));
            gcode.process_subcommands_now(PSTR("G1 F800 Z10"));
            // gcode.process_subcommands_now(PSTR("G1 F500 E50"));
            // gcode.process_subcommands_now(PSTR("G1 F500 E-200"));
            gcode.process_subcommands_now(PSTR("M400"));
            // gcode.process_subcommands_now(PSTR("G4 S30"));
            // gcode.process_subcommands_now(PSTR("M104 S0"));

        }
        if(lcd_info.level_flag!=1)
          lcd_info.reach_flag=1;
      // }
      //  DWIN_UpdateLCD();
}

void DGUSScreenHandler::HandleManualhotend(void)
{

    if(lcd_info.set_hoted_flag==1)
    {
         if(!ExtUI::isPrintingFromMedia())
         {
          if(thermalManager.still_heating(0)==0)
          {
                const millis_t ms = millis();
                static millis_t next_check_ms = 0;
                if (ELAPSED(ms, next_check_ms)) {
                lcd_info.reach_cnt++;
                // DEBUG_ECHOLNPGM(" lcd_info.reach_cnt", lcd_info.reach_cnt);
                if(lcd_info.reach_cnt>600)
                {

                    thermalManager.setTargetHotend(0, 0);
                    thermalManager.setTargetBed(0);
                    lcd_info.reach_cnt=0;
                    lcd_info.set_hoted_flag=0;
                }
                next_check_ms = ms + 1000UL;
                }
          }
         }
    }
...
void DGUSScreenHandler::HandleLoad(DGUS_VP_Variable &var, void *val_ptr) {
    load_flag=1;
    lcd_info.set_hoted_flag = 0;
    lcd_info.reach_cnt = 0;
    ...
    if(lcd_info.language == (language_t)1)
    {
       GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_LOAD);
    }
    else if(lcd_info.language == (language_t)2)
    {
      GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_EN_LOAD);
    }
    else if(lcd_info.language == (language_t)3)
    {
      GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_ES_LOAD);
    }
    else if(lcd_info.language == (language_t)4)
    {
      GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_FR_LOAD);
    }
    else if(lcd_info.language == (language_t)5)
    {
      GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_DE_LOAD);
    }
    else if(lcd_info.language == (language_t)6)
    {
      GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_PY_LOAD);
    }
}

void DGUSScreenHandler::HandleUnload(DGUS_VP_Variable &var, void *val_ptr) {
    unload_flag=1;
    lcd_info.set_hoted_flag = 0;
    lcd_info.reach_cnt = 0;
    queue.inject_P(PSTR("M140 S0"));
    ...
    if(lcd_info.language == (language_t)1)
    {
       GotoScreen((DGUSLCD_Screens)DGUSLCD_SCREEN_UNLOAD);
    }
    else if(lcd_info.language == (language_t)2)
    {
        GotoScreen((DGUSLCD_Screens)(73));
    }
    else if(lcd_info.language == (language_t)3)
    {
        GotoScreen((DGUSLCD_Screens)(109));
    ...
    {
        GotoScreen((DGUSLCD_Screens)(147));
    }
    else if(lcd_info.language == (language_t)5)
    {
        GotoScreen((DGUSLCD_Screens)(183));
    }
     else if(lcd_info.language == (language_t)6)
    {
        GotoScreen((DGUSLCD_Screens)(220));
    }
}

void DGUSScreenHandler::HandleCooldown(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    thermalManager.disable_all_heaters();
    dgusdisplay.WriteVariable(VP_E0_CONTROL,(uint16_t) 0);
    dgusdisplay.WriteVariable(VP_BED_CONTROL,(uint16_t) 0);
    dgusdisplay.WriteVariable(VP_Fan_SET, (uint16_t)0);
}
...
void DGUSScreenHandler::HandleMaxAccelerationChanged(DGUS_VP_Variable &var, void *val_ptr) {
 uint16_t value_raw = swap16(*(uint16_t*)val_ptr);

  ExtUI::axis_t axis;
  switch (var.VP) {
    case VP_X_MAX_ACC: axis = ExtUI::axis_t::X; break;
    case VP_Y_MAX_ACC: axis = ExtUI::axis_t::Y; break;
    case VP_Z_MAX_ACC: axis = ExtUI::axis_t::Z; break;
    default: return;
  }

  ExtUI::setAxisMaxAcceleration_mm_s2(value_raw, axis);
  settings.save();
...
void DGUSScreenHandler:: HandleMaxAccelerationChanged_extruder(DGUS_VP_Variable &var, void *val_ptr){
  ...
  switch (var.VP) {
    case VP_E_MAX_ACC: extruder = ExtUI::extruder_t::E0; break;
    default: return;
  }
  ExtUI::setAxisMaxAcceleration_mm_s2(value_raw, extruder);
  settings.save();
...
void DGUSScreenHandler::HandleMAXSpeedChanged(DGUS_VP_Variable &var, void *val_ptr) {
  // DEBUG_ECHOLNPGM("HandleMAXSpeedChanged");
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);
  // DEBUG_ECHOLNPGM("value_raw:", value_raw);
  ...
  // DEBUG_ECHOLNPGM("value:", value_raw);
  ExtUI::setAxisMaxFeedrate_mm_s(value_raw, axis);
  settings.save();
...
void DGUSScreenHandler::HandleMAXSpeedChanged_extruder(DGUS_VP_Variable &var, void *val_ptr) {
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);

   ExtUI::extruder_t extruder;
  switch (var.VP) {
      case VP_E_MAX_SPEED: extruder = ExtUI::extruder_t::E0; break;
    default: return;
  }
    // DEBUG_ECHOLNPGM("value:", value_raw);
  ExtUI::setAxisMaxFeedrate_mm_s(value_raw, extruder);
  settings.save();
  // DEBUG_ECHOLNPAIR_F("value_set:", ExtUI::getAxisMaxFeedrate_mm_s(axis));
  skipVP = var.VP; // don't overwrite value the next update time as the display might autoincrement in parallel
  return;
...
#define PLA_HOTED_EEPROM_ADDRESS 0x02   // Between 0x01 and 0x63 (EEPROM_OFFSET-1)
#define PLA_BED_EEPROM_ADDRESS 0x05   // Between 0x01 and 0x63 (EEPROM_OFFSET-1)
#define ABS_HOTED_EEPROM_ADDRESS 0x08   // Between 0x01 and 0x63 (EEPROM_OFFSET-1)
#define ABS_BED_EEPROM_ADDRESS 0x0B   // Between 0x01 and 0x63 (EEPROM_OFFSET-1)


void GetPreparePLAHoted(void)
{
    BL24CXX::read(PLA_HOTED_EEPROM_ADDRESS, (uint8_t*)&lcd_info.preheat_pla_hoted, sizeof(lcd_info.preheat_pla_hoted));

}
void GetPreparePLABed(void)
{
    BL24CXX::read(PLA_BED_EEPROM_ADDRESS, (uint8_t*)&lcd_info.preheat_pla_bed, sizeof(lcd_info.preheat_pla_bed));
}

void GetPrepareABSHoted(void)
{
    BL24CXX::read(ABS_HOTED_EEPROM_ADDRESS, (uint8_t*)&lcd_info.preheat_abs_hoted, sizeof(lcd_info.preheat_abs_hoted));
}
void GetPrepareABSBed(void)
{
  BL24CXX::read(ABS_BED_EEPROM_ADDRESS, (uint8_t*)&lcd_info.preheat_abs_bed, sizeof(lcd_info.preheat_abs_bed));
}

void DGUSScreenHandler::HandlePreparePLAHotedChanged(DGUS_VP_Variable &var, void *val_ptr) {
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);
  BL24CXX::write(PLA_HOTED_EEPROM_ADDRESS, (uint8_t*)&value_raw, sizeof(value_raw));
  BL24CXX::read(PLA_HOTED_EEPROM_ADDRESS, (uint8_t*)&lcd_info.preheat_pla_hoted, sizeof(lcd_info.preheat_pla_hoted));
}
void DGUSScreenHandler::HandlePreparePLABedChanged(DGUS_VP_Variable &var, void *val_ptr) {
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);
  BL24CXX::write(PLA_BED_EEPROM_ADDRESS, (uint8_t*)&value_raw, sizeof(value_raw));
}
void DGUSScreenHandler::HandlePrepareABSHotedChanged(DGUS_VP_Variable &var, void *val_ptr) {
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);
  BL24CXX::write(ABS_HOTED_EEPROM_ADDRESS, (uint8_t*)&value_raw, sizeof(value_raw));
}
void DGUSScreenHandler::HandlePrepareABSBedChanged(DGUS_VP_Variable &var, void *val_ptr) {
  const uint16_t value_raw = BE16_P(val_ptr);
  // uint16_t value_raw = swap16(*(uint16_t*)val_ptr);
  BL24CXX::write(ABS_BED_EEPROM_ADDRESS, (uint8_t*)&value_raw, sizeof(value_raw));
}
...
void DGUSScreenHandler::HandleHeaterControl(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  const uint16_t preheat_temp = BE16_P(val_ptr);
  // int16_t preheat_temp = swap16(*(uint16_t*)val_ptr);
   DEBUG_ECHOLNPGM(" >> ", preheat_temp);
  ...
  void DGUSScreenHandler::HandlePowerLossRecovery(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    if (value) {
      ...
      dgusdisplay.WriteVariable(VP_SHOW_PRINT_FILE, filelist.filename(), 32, true);
    }
    else {
      recovery.cancel();
    }
...
void DGUSScreenHandler::UpdateScreenVPData() {
  ...
  if (!VPList) {
    // DEBUG_ECHOPGM(" NO SCREEN FOR: ", current_screen);
    ScreenComplete = true;
  ...
  do {
    ...
    if (!VP) {
      update_ptr = 0;
      // DEBUG_ECHOPGM(" UpdateScreenVPData done");
      ScreenComplete = true;
