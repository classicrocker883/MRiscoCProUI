...
MenuClass *MaxAccelMenu = nullptr;
 ...
 || ANY(MPC_EDIT_MENU, MPC_AUTOTUNE_MENU)
  ...
  MenuClass *HomingFRMenu = nullptr;
#endif
...
TERN_(HAS_PROUI_MESH_EDIT, MenuItemClass *EditZValueItem   = nullptr;)
...
void Popup_window_PauseOrStop() {
  switch (select_print.now) {
    case PRINT_PAUSE_RESUME:
      Popup_ConfirmCancel(ICON_Pause_1, GET_TEXT_F(MSG_PAUSE_PRINT));
      break;
    case PRINT_STOP:
      Popup_ConfirmCancel(ICON_Stop_1, GET_TEXT_F(MSG_STOP_PRINT));
      break;
    default: break;
  }
}
...
void ICON_ResumeOrPause() {
  ...
  if (print_job_timer.isPaused() || HMI_flag.pause_flag) {
    ICON_Resume();
  }
  else {
    ICON_Pause();
...
void _draw_feedrate() {
    ...
    if (HMI_data.SpdInd) {
      ...
      if (blink && _should_redraw == true) {
        DWINUI::Draw_String(DWIN_FONT_STAT, HMI_data.Indicator_Color, HMI_data.Background_Color, 116 + 4 * STAT_CHR_W + 2, 384, F(" %"));
        DWINUI::Draw_Int(DWIN_FONT_STAT, HMI_data.Indicator_Color, HMI_data.Background_Color, 3, 116 + 2 * STAT_CHR_W, 384, motion.feedrate_percentage);
      }
      else if (_feedrate != motion.feedrate_percentage) {
...
void onClickSDItem() {
  ...
  if (hasUpDir && CurrentMenu->selected == 1) {
    ...
    if (card.fileIsBinary()) {
      Popup_Confirm(ICON_Error, GET_TEXT_F(MSG_CHECK_FILENAME), GET_TEXT_F(MSG_ONLY_GCODE));
      return;
    }
    else {
...
void Draw_Info_Menu() {
  ...
  Draw_Menu_Line(0, ICON_Back, GET_TEXT_F(MSG_BUTTON_BACK), false, true);
...
// Plot Temperature Graph (PID Tuning Graph)
  ...
  void DWIN_PIDTuning(tempcontrol_t result) {
    ...
    switch (result) {
      ...
      default:
        checkkey = last_checkkey;
        Popup_Confirm(ICON_Info_0, GET_TEXT_F(MSG_ERROR), GET_TEXT_F(MSG_STOPPING));
        break;
    }
  }
  ...
  void DWIN_MPCTuning(tempcontrol_t result) {
    ...
    switch (result) {
      ...
      case MPC_TEMP_ERROR:
        checkkey = last_checkkey;
        Popup_Continue(ICON_TempTooHigh, GET_TEXT_F(MSG_PID_AUTOTUNE_FAILED), F(STR_MPC_TEMPERATURE_ERROR));
        ui.reset_alert_level();
        break;
      case MPC_INTERRUPTED:
        checkkey = last_checkkey;
        Popup_Continue(ICON_TempTooHigh, GET_TEXT_F(MSG_ERROR), F(STR_MPC_AUTOTUNE_INTERRUPTED));
        ui.reset_alert_level();
        break;
      case AUTOTUNE_DONE:
        checkkey = last_checkkey;
        Popup_Confirm(ICON_TempTooLow, GET_TEXT_F(MSG_MPC_AUTOTUNE), GET_TEXT_F(MSG_BUTTON_DONE));
        ui.reset_alert_level();
        break;
      default:
        checkkey = last_checkkey;
...
void DWIN_HomingStart() {
  ...
  DWIN_Show_Popup(HOME_AND_KILL_ICON, GET_TEXT_F(MSG_HOMING), GET_TEXT_F(MSG_PLEASE_WAIT));
}
...
void DWIN_Print_Finished() {
  ...
  if (auto_abort) {
    if (motion.all_axes_homed()) {
      ...
      #else
        const float zpos = motion.position.z + TERN(NOZZLE_PARK_FEATURE, NOZZLE_PARK_Z_RAISE_MIN, Z_POST_CLEARANCE);
        _MIN(zpos, Z_MAX_POS);
...
void DWIN_SetDataDefaults() {
  ...
  TERN_(PROUI_ITEM_CONF, HMI_data.auto_confirm = true;)
    ...
    PRO_data.z_max_pos  = DEF_Z_MAX_POS;
      ...
      PRO_data.mesh_min_x = mesh_min.x;
      PRO_data.mesh_max_x = mesh_max.x;
      PRO_data.mesh_min_y = mesh_min.y;
      PRO_data.mesh_max_y = mesh_max.y;
    #endif
    #if HAS_BED_PROBE
    ...
    TERN_(HAS_FILAMENT_SENSOR, PRO_data.Runout_active_state = FIL_RUNOUT_STATE;)
    ...
    TERN_(MESH_BED_LEVELING, SetData();)
  #endif
}
...
void MarlinUI::kill_screen(FSTR_P const lcd_error, FSTR_P const) {
  DWIN_Show_Popup(HOME_AND_KILL_ICON, GET_TEXT_F(MSG_PRINTER_KILLED), lcd_error);
  ...
  void MarlinUI::pause_show_message(const PauseMessage message, const PauseMode mode/*=PAUSE_MODE_SAME*/, const uint8_t extruder/*=EXT*/) {
    ...
    switch (message) {
      case PAUSE_MESSAGE_PARKING:  Popup_Pause(GET_TEXT_F(MSG_PAUSE_PRINT_PARKING));    break; // M125
      case PAUSE_MESSAGE_CHANGING: Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_INIT));   break; // pause_print (M125, M600)
      case PAUSE_MESSAGE_WAITING:  Popup_Pause(GET_TEXT_F(MSG_ADVANCED_PAUSE_WAITING), BTN_Continue); break;
      case PAUSE_MESSAGE_UNLOAD:   Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_UNLOAD)); break; // Unload of pause and Unload of M702
      case PAUSE_MESSAGE_INSERT:   Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_INSERT), BTN_Continue); break;
      case PAUSE_MESSAGE_LOAD:     Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_LOAD));   break;
      case PAUSE_MESSAGE_PURGE:    Popup_Pause(GET_TEXT_F(TERN(ADVANCED_PAUSE_CONTINUOUS_PURGE, MSG_FILAMENT_CHANGE_CONT_PURGE, MSG_FILAMENT_CHANGE_PURGE))); break;
      case PAUSE_MESSAGE_OPTION:   Goto_FilamentPurge(); break;
      case PAUSE_MESSAGE_RESUME:   Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_RESUME)); break;
      case PAUSE_MESSAGE_STATUS:   HMI_ReturnScreen(); break;                                  // Exit from Pause, Load and Unload
      case PAUSE_MESSAGE_HEAT:     Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEAT), BTN_Continue); break;
      case PAUSE_MESSAGE_HEATING:  Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEATING)); break;
      default: break;
    }
  }
...
void AxisMove(AxisEnum axis) {
    ...
    if (axis == E_AXIS && thermalManager.tooColdToExtrude(EXT)) {
      ...
      Popup_Confirm(ICON_TempTooLow, GET_TEXT_F(MSG_HOTEND_TOO_COLD), GET_TEXT_F(MSG_PLEASE_PREHEAT));
      return;
    }
  #endif
...
void ApplyMove() {
...
#if HAS_HOME_OFFSET
  void ApplyHomeOffset() { motion.set_home_offset(HMI_value.axis, MenuData.Value / MINUNITMULT); }
  ...
  // Mesh Popup
  void PopUp_StartAutoLev() { Popup_ConfirmCancel(ICON_Leveling_1, GET_TEXT_F(MSG_START_ABL)); }
...
// Park or Raise Head
  ...
  void RaiseHead() {
    LCD_MESSAGE(MSG_TOOL_CHANGE_ZLIFT);
    gcode.process_subcommands_now(TS(F("G0F600Z"), float(_MIN(motion.position.z + (Z_POST_CLEARANCE), Z_MAX_POS, F("\nM400")))));
  }
#endif
  ...
  void DWIN_MeshViewer() {
    if (!leveling_is_valid()) {
      Popup_Confirm(ICON_Leveling_1, GET_TEXT_F(MSG_MESH_VIEWER), GET_TEXT_F(MSG_NO_VALID_MESH));
...
void Goto_ConfirmToPrint() {
    ...
    if (fileprop.isLaser) {
      if (laser_device.is_laser_device()) {
        Draw_LaserPrint_Menu();
        return;
      }
      else {
        Draw_LaserSettings_Menu();
        return;
      }
    }
    else
    ...
    if (HMI_data.EnablePreview) {
      Goto_Popup(preview.drawFromSD, OnClick_ConfirmToPrint);
      return;
    }
  #endif
...
void ApplyMaxAccel() { planner.set_max_acceleration(HMI_value.axis, MenuData.Value); }
...
#if HAS_X_AXIS
  void SetStepsX() { SetPFloatOnClick(min_steps_edit_values.x, max_steps_edit_values.x, 2); }
#endif
#if HAS_Y_AXIS
  void SetStepsY() { SetPFloatOnClick(min_steps_edit_values.y, max_steps_edit_values.y, 2); }
#endif
#if HAS_Z_AXIS
  void SetStepsZ() { SetPFloatOnClick(min_steps_edit_values.z, max_steps_edit_values.z, 2); }
#endif
#if HAS_HOTEND
  void SetStepsE() { SetPFloatOnClick(min_steps_edit_values.e, max_steps_edit_values.e, 2); }
#endif
  ...
  #if HAS_X_AXIS
    void SetHomingX() { HMI_value.axis = X_AXIS; SetIntOnClick(min_homing_edit_values.x, max_homing_edit_values.x, motion.homing_feedrate_mm_m.x, ApplyHomingFR); }
  #endif
  #if HAS_Y_AXIS
    void SetHomingY() { HMI_value.axis = Y_AXIS; SetIntOnClick(min_homing_edit_values.y, max_homing_edit_values.y, motion.homing_feedrate_mm_m.y, ApplyHomingFR); }
  #endif
  #if HAS_Z_AXIS
    void SetHomingZ() { HMI_value.axis = Z_AXIS; SetIntOnClick(min_homing_edit_values.z, max_homing_edit_values.z, motion.homing_feedrate_mm_m.z, ApplyHomingFR); }
  #endif
#endif
  ...
  void ApplyPhySet() {
    bedlevel.initialize();
    motion.update_software_endstops(X_AXIS);
    motion.update_software_endstops(Y_AXIS);
...
void Draw_Move_Menu() {
  ...
  if (SET_MENU(MoveMenu, MSG_MOVE_AXIS, 6)) {
    BACK_ITEM(Draw_Prepare_Menu);
    #if HAS_X_AXIS
      EDIT_ITEM(ICON_MoveX, MSG_MOVE_X, onDrawPFloatMenu, SetMoveX, &motion.position.x);
    #endif
    #if HAS_Y_AXIS
      EDIT_ITEM(ICON_MoveY, MSG_MOVE_Y, onDrawPFloatMenu, SetMoveY, &motion.position.y);
    #endif
    #if HAS_Z_AXIS
      EDIT_ITEM(ICON_MoveZ, MSG_MOVE_Z, onDrawPFloatMenu, SetMoveZ, &motion.position.z);
    #endif
    #if HAS_HOTEND
      gcode.process_subcommands_now(F("G92E0")); // Reset extruder position
      EDIT_ITEM(ICON_Extruder, MSG_MOVE_E, onDrawPFloatMenu, SetMoveE, &motion.position.e);
    #endif
  ...
  void Draw_ProbeSet_Menu() {
    checkkey = Menu;
    if (SET_MENU(ProbeSetMenu, MSG_ZPROBE_SETTINGS, 10)) {
...
void Draw_Tune_Menu() {
  #if ENABLED(CV_LASER_MODULE)
    if (laser_device.is_laser_device()) {
      LCD_MESSAGE_F("Not available in laser mode");
      return;
    }
  #endif
  checkkey = Menu;
...
//=============================================================================
  ...
  void Draw_ChamberPID_Menu() {
    ...
    if (SET_MENU(ChamberPIDMenu, MSG_CHAMBER_PID_SETTINGS, 7)) {
      ...
      #endif
      #if ENABLED(PID_EDIT_MENU)
        EDIT_ITEM(ICON_PIDValue, MSG_PID_SET_KP, onDrawPFloat2Menu, SetKp, &thermalManager.temp_chamber.pid.Kp);
        EDIT_ITEM(ICON_PIDValue, MSG_PID_SET_KI, onDrawPIDi, SetKi, &thermalManager.temp_chamber.pid.Ki);
        EDIT_ITEM(ICON_PIDValue, MSG_PID_SET_KD, onDrawPIDd, SetKd, &thermalManager.temp_chamber.pid.Kd);
      #endif
    }
...
//=============================================================================
  ...
  //void CreatePlaneFromMesh() { bedLevelTools.create_plane_from_mesh(); }
  #if PROUI_EX
    void ApplyMeshPoints() { PRO_data.grid_max_points = MenuData.Value; TERN_(HAS_PROUI_MESH_EDIT, ResetMeshInset();) ReDrawMenu(); }
    ...
    void ApplyMeshPoints() { HMI_data.grid_max_points = MenuData.Value; TERN_(HAS_PROUI_MESH_EDIT, ResetMeshInset();) ReDrawMenu(); }
    void LiveMeshPoints() { DrawMeshPoints(true, CurrentMenu->line(), MenuData.Value); }
    void SetMeshPoints() {
      SetOnClick(SetIntNoDraw, GRID_MIN, GRID_LIMIT, 0, HMI_data.grid_max_points, ApplyMeshPoints, LiveMeshPoints);
      DrawMeshPoints(true, CurrentMenu->line(), HMI_data.grid_max_points);
    }
  #endif
    ...
    void Popup_ResetMesh() { Popup_ConfirmCancel(ICON_Info_0, GET_TEXT_F(MSG_RESET_MESH)); }
    ...
    void Draw_EditMesh_Menu() {
      if (!leveling_is_valid()) {
        ...
        Popup_Confirm(ICON_Leveling_1, GET_TEXT_F(MSG_NO_VALID_MESH), GET_TEXT_F(MSG_UBL_LOAD_MESH));
        return;
      }
...
//=============================================================================
    ...
    void SetHome() {
      laser_device.homepos += motion.position;
  ...
  void Draw_LaserSettings_Menu() {
    ...
    if (SET_MENU(LaserSettings, MSG_LASER_MENU, 7)) {
      ...
      MENU_ITEM(ICON_Homing, MSG_AUTO_HOME, onDrawMenuItem, AutoHome);
      EDIT_ITEM_F(ICON_LaserFocus, "Laser Focus", onDrawPFloatMenu, SetMoveZ, &motion.position.z);
      EDIT_ITEM(ICON_MoveX, MSG_MOVE_X, onDrawPFloatMenu, SetMoveX, &motion.position.x);
      EDIT_ITEM(ICON_MoveY, MSG_MOVE_Y, onDrawPFloatMenu, SetMoveY, &motion.position.y);
...
#if ENABLED(HOST_SHUTDOWN_MENU_ITEM) && defined(SHUTDOWN_ACTION)
  void PopUp_HostShutDown() { Popup_ConfirmCancel(ICON_Info_1, GET_TEXT_F(MSG_HOST_SHUTDOWN)); }
...
void Draw_AdvancedSettings_Menu() {
  ...
  if (SET_MENU(AdvancedSettings, MSG_MESH_LEVELING, 10)) {
    ...
    MMeshMoveZItem = EDIT_ITEM(ICON_Zoffset, MSG_MESH_EDIT_Z, onDrawPFloat2Menu, SetMMeshMoveZ, &motion.position.z);
