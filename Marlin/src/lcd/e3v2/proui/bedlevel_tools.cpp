    ...
    GRID_LOOP(x, y) {
      ...
      */
      ...
      if (isnan(z)) { // Undefined
        DWIN_Draw_String(false, Title.meshfont, DWINUI::textcolor, DWINUI::backcolor, start_x_px + cell_width_px / 2 - 5, start_y_px + offset_y, F("X"));
      }
      else {          // Has value
        MString<12> msg;
        ...
        if ((GRID_MAX_POINTS_X) >= TERN(TJC_DISPLAY, 8, 10))
          DWIN_Draw_String(false, Title.meshfont, DWINUI::textcolor, DWINUI::backcolor, start_x_px - 2 + offset_x, start_y_px + offset_y, F("."));
        DWIN_Draw_String(false, Title.meshfont, DWINUI::textcolor, DWINUI::backcolor, start_x_px + 1 + offset_x, start_y_px + offset_y, msg);
      }
      safe_delay(10);
      LCD_SERIAL.flushTX();
    } // GRID_LOOP
