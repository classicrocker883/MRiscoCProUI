...
bool Stepper::disable_axis(const AxisEnum axis) {
  ...
  TERN_(DWIN_LCD_PROUI, motion.set_axis_never_homed(axis)); // MRISCOC workaround: https://github.com/MarlinFirmware/Marlin/issues/23095
...
void Stepper::apply_directions() {
   ...
   */
    ...
    FORCE_INLINE int32_t Stepper::_eval_bezier_curve(const uint32_t curr_step) {
      #if (defined(__arm__) || defined(__thumb__)) && __ARM_ARCH >= 7 && !defined(STM32G0B1xx)
