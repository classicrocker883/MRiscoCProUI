...
public:
  ...
  static void move_z_after_probing() {
    DEBUG_SECTION(mzah, "move_z_after_probing", DEBUGGING(LEVELING));
      ...
      motion.do_z_clearance(Z_AFTER_PROBING, true); // Move down still permitted
    #endif
  }
...
private:
    ...
    static float run_z_probes(const bool sanity_check=true, const float z_min_point=Z_PROBE_LOW_POINT, const float z_clearance=Z_TWEEN_SAFE_CLEARANCE);
  #endif
};
