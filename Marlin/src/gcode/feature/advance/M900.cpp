...
void GcodeSuite::M900_report(const bool forReplay/*=true*/) {
  ...
  report_heading(forReplay, F(STR_LINEAR_ADVANCE));
    ...
    SERIAL_ECHOPGM("  M900 K", p_float_t(planner.extruder_advance_K[0], 3));
    #if ENABLED(SMOOTH_LIN_ADVANCE)
      SERIAL_ECHOPGM("  M900 U", p_float_t(stepper.get_advance_tau(), 4));
    #endif
    SERIAL_EOL();
  #else
    EXTRUDER_LOOP() {
      report_echo_start(forReplay);
      SERIAL_ECHOPGM("  M900 T", p_float_t(e, 3), " K", p_float_t(planner.extruder_advance_K[e], 3));
      #if ENABLED(SMOOTH_LIN_ADVANCE)
        SERIAL_ECHOPGM(" U", p_float_t(stepper.get_advance_tau(e), 4));
      #endif
      SERIAL_EOL();
    }
  #endif
